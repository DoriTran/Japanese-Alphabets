...
export type ResponsivePoint = "phone" | "tablet" | "middle" | "desktop" | "screen" | "extra";
...
export default function useResponsivePoint(): ResponsivePointHook {
  ...
  const isSmallerOrEqual = (point: ResponsivePoint): boolean => screenWidth <= themes.breakpoints[point];
  const isLarger = (point: ResponsivePoint): boolean => screenWidth > themes.breakpoints[point];
  const isLargerOrEqual = (point: ResponsivePoint): boolean => screenWidth >= themes.breakpoints[point];
  ...
  return { responsivePoint, isSmaller, isSmallerOrEqual, isLarger, isLargerOrEqual, is };
}
